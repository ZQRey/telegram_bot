...
redak_info_markup = telebot.types.ReplyKeyboardMarkup(True, True)
redak_info_markup.add('Номер телефона')
redak_info_markup.add('Электронная почта')
...
def menu_item(message):
    ...
    elif message.text.lower() == 'Изменить информацию о себе':
        msg = bot.send_message(chat_id, 'Выберите пункт ниже, какую информацию вы хотите изменить.',
                               reply_markup=redak_info_markup)
        bot.register_next_step_handler(msg, red_info)
    else:
...
def red_info(msg):
    if msg.text.lower() == 'номер телефона':
        pass
    elif msg.text.lower() == 'электронная почта':
        pass
